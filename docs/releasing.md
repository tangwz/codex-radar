...
`v<MARKETING_VERSION>` tag 一经推送，tag 名编码的 App Version 和 tag 就永久被该次尝试占用；tag commit 中可合法解析的 build number 也永久占用。标准发布提交必须包含与 tag 匹配的合法 `version.env`，因此正常 Candidate 的三项标识会同时 burned。`prepare-candidate` 只允许 tag push 的第一次 workflow attempt 进入发布路径，GitHub Actions rerun 会在签名和 Draft Release 创建前被拒绝。即使 workflow 在创建 Draft Release 前失败，也不删除或重跑该 tag；修复必须同时提高 `MARKETING_VERSION` 和 `BUILD_NUMBER`，合入 `main` 后创建匹配的新 tag。

`Prepare Update Candidate` 与 `Publish Update` 共用仓库级 `update-${{ github.repository }}` concurrency group，同一时刻只允许一个发布 workflow 推进，并通过 `queue: max` 保留最多 100 个等待中的发布 run，而不是让新 run 淘汰已有 pending run。每个 Candidate 都会在签名前要求新 App Version 严格高于所有其他语法合法的受保护 `vX.Y.Z` tag 名，并要求新 build number 严格高于这些 tag commit 中所有可合法解析的 build number。tag 名是历史 App Version 的唯一 canonical source；`version.env` 中不匹配的 App Version 不参与比较，避免一个不可删除的错误 tag 永久锁死后续发布。缺失或损坏的配置仍会永久占用 tag 名编码的 App Version，但它在 Candidate identity 校验前已经失败，不存在可信的 build identity，流程不得猜测一个 build number。该门禁不因 Production Feed 已存在而跳过。校验成功即建立该 Candidate 的 identity reservation：之后创建的 tag 必须在自己的签名前校验中高于它，但不会追溯性地使已经签名的 Candidate 失效。公开与激活阶段只验证已保留 Draft、tag、资产和 feed CAS，不重新推断 tag 创建顺序。
