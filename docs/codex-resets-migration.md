...
- 有效快照中的较新预测会静默消费同批已观察到的公告，避免预测过期后补发被覆盖的旧公告；该状态跨重启保留，不受预测通知投递是否成功影响。失效快照不消费公告，后续真正的新公告仍正常通知。
- 分页游标保持不透明，不做解码或推断。[公开 OpenAPI](https://codex-resets.com/api/openapi.json) 的 `cursor` 参数明确限制为 1–1024 字符和 `^[A-Za-z0-9_-]+$`，客户端保留与该契约一致的校验。
...
- 应用重启后，首次历史加载会条件重验证所有分页，因为内存中的公告版本关联尚未建立。若重验证失败则显示历史不可用，不把尚未确认与当前公告一致的磁盘缓存当作最新统计。
